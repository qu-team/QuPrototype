...
public class L10N : MonoBehaviour {
    ...
    private class Italian : Language {
        public Italian() : base(new English(), new Dictionary<Label, string>() {
            ...
            { Label.CARD_11_DESCRIPTION, @"Alcune persone hanno difficoltà a distinguere determinati colori, in particolare rosso e verde." },
            ...
            { Label.CARD_13_DESCRIPTION, @"La luce bianca può essere divisa nelle sue colorate componenti usando un prisma: un oggetto con angoli particolari in grado di cambiare la direzione della luce in base alla lunghezza d'onda" },
            { Label.CARD_13_TASK, "Completa l'ultimo livello" },
			{ Label.CARD_14_TITLE, "Schermi e carta " },
            { Label.CARD_14_DESCRIPTION, @"Gli artisti che lavorano in digitale devono sempre controllare il loro lavoro nello spazio di colore CYMK, che è quello usato dalle stampanti. Gli schermi lavorano in RGB." },
            { Label.CARD_14_TASK, "Guarda tutti i filmati in una volta sola nella sezione 'Ricordi'" },
			{ Label.CARD_15_TITLE, "Comunicazione" },
            { Label.CARD_15_DESCRIPTION, @"I colori giocano un importantissimo ruolo nella comunicazione. Molti designers sanno come sfruttarli per rendere più attraente un prodotto per determinate persone." },
            { Label.CARD_15_TASK, "Salva almeno 40 qU in ogni livello" },
			{ Label.CARD_16_TITLE, "Caldo e freddo" },
            { Label.CARD_16_DESCRIPTION, @"Generalmente i colori con una lunghezza d'onda maggiore, vengono detti 'caldi', mentre gli altri vengono definiti 'freddi'. In pochi sanno che più un oggetto è caldo, più vira verso il blu la luce che emette" },
            { Label.CARD_16_TASK, "Gioca per almeno 3 ore (in totale)" },
			{ Label.CARD_17_TITLE, "Fuochi d'artificio" },
            { Label.CARD_17_DESCRIPTION, @"Il colore dei fuochi d'artificio è determinato dalla composizione chimica della polvere che brucia: Cloruro di litio per il magenta, sali di rame per blu e verde, ecc." },
            { Label.CARD_17_TASK, "Salva 100 qU in totale" },
			{ Label.CARD_18_TITLE, "Spin dei qU" },
            { Label.CARD_18_DESCRIPTION, @"Sapresti dire quale dei due qU è a testa in giù?" },
            { Label.CARD_18_TASK, "Ottieni più di 500 punti in un livello" },
